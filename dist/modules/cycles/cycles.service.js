...
class CyclesService {
    static async getCyclePredictions(userId, relativeDateStr) {
        ...
        const [settings, recentPeriods] = await Promise.all([
            ...
            prisma_1.prisma.periodLog.findMany({
        ...
        if (recentPeriods.length > 1) {
            ...
            if (validCycles > 0)
                avgCycleLength = Math.round(totalCycleDays / validCycles);
            if (validPeriods > 0)
                avgPeriodLength = Math.round(totalPeriodDays / validPeriods);
        }
        else {
